...
$db = getDB();
$total = $db->query("SELECT COUNT(*) FROM appointments")->fetch_row()[0];
...
$appointments = $db->query("SELECT a.*, u.full_name, u.phone FROM appointments a JOIN users u ON a.user_id = u.id ORDER BY a.appointment_date DESC LIMIT 50")->fetch_all(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
...
<a class="bg-secondary-container text-on-secondary-container rounded-lg px-4 py-3 mx-2 flex items-center gap-3 scale-95" href="appointments.php"><span class="material-symbols-outlined">calendar_month</span><span class="text-label-md">Appointments</span></a>
...
<button class="flex items-center gap-2 bg-primary text-on-primary px-6 py-3 rounded-full text-label-md shadow-sm hover:opacity-90 transition-opacity">
...
<p class="text-headline-md text-headline-md text-on-surface mt-1"><?= $total ?></p>
</div>
<div class="bento-card bg-surface-container-lowest p-5 rounded-xl border border-outline-variant">
<span class="text-label-sm text-on-surface-variant uppercase tracking-widest">Confirmed</span>
<p class="text-headline-md text-headline-md text-primary mt-1"><?= $confirmed ?></p>
</div>
<div class="bento-card bg-surface-container-lowest p-5 rounded-xl border border-outline-variant">
<span class="text-label-sm text-on-surface-variant uppercase tracking-widest">Pending</span>
<p class="text-headline-md text-headline-md text-secondary mt-1"><?= $pending ?></p>
</div>
<div class="bento-card bg-surface-container-lowest p-5 rounded-xl border border-outline-variant">
<span class="text-label-sm text-on-surface-variant uppercase tracking-widest">Cancelled</span>
<p class="text-headline-md text-headline-md text-on-error-container mt-1"><?= $cancelled ?></p>
</div>
</section>
...
<input class="bg-surface border-none text-label-sm rounded-lg focus:ring-primary pl-9 py-2 w-56" placeholder="Search patient..." type="text"/>
</div>
</div>
...
$type_class = $a['appointment_type'] === 'video' ? 'bg-primary-fixed text-on-primary-fixed' : 'bg-tertiary-container text-on-tertiary';
?>
<tr>
<td class="px-6 py-4">
...
<p class="text-label-sm text-on-surface-variant"><?= htmlspecialchars($a['phone'] ?? '') ?></p>
</div>
</div>
</td>
<td class="px-6 py-4 text-body-md"><?= htmlspecialchars($a['doctor_name'] ?? '—') ?></td>
<td class="px-6 py-4 text-body-md"><?= date('M d, Y · h:i A', strtotime($a['appointment_date'])) ?></td>
<td class="px-6 py-4"><span class="px-2 py-1 rounded text-label-sm <?= $type_class ?>"><?= ucfirst($a['appointment_type'] ?? 'video') ?></span></td>
<td class="px-6 py-4"><span class="px-3 py-1 rounded-full text-label-sm <?= $status_class ?>"><?= ucfirst($a['status']) ?></span></td>
...
<tr><td colspan="6" class="px-6 py-8 text-center text-on-surface-variant">No appointments found.</td></tr>
<?php endif; ?>
</tbody>
</table>
...
<span>Showing <?= count($appointments) ?> of <?= $total ?></span>
